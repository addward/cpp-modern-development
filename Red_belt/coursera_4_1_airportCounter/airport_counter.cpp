...
public:
  ...
  AirportCounter() {
    ...
    //airports = vector<pair<TAirport, size_t>>(size);
...
template <typename TAirport> template <typename TIterator>
AirportCounter<TAirport>::AirportCounter(TIterator begin, TIterator end) : AirportCounter<TAirport>::AirportCounter(){
...
template <typename TAirport>
array<pair<TAirport, size_t>, static_cast<size_t>(TAirport::Last_)> AirportCounter<TAirport>::GetItems() const{
    return airports;
}
