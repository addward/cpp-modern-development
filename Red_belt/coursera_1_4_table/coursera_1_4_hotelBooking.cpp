...
public:
    void addGuest(uint64_t time, const string& hotelName, uint32_t guestId, uint16_t roomCount);
    void clearOldBookings(uint64_t time);
    int uniqueClientsNumber(const string& hotel_name) const ;
    int bookedRoomsNumber(const string& hotel_name) const ;
    void printDb();
};

void BookingDatabase::addGuest(uint64_t time, const string& hotelName, uint32_t guestId, uint16_t roomCount){
...
void BookingDatabase::clearOldBookings(uint64_t time){
...
int BookingDatabase::bookedRoomsNumber(const string& hotel_name) const {
    if (hotel_guests_.count(hotel_name) == 0) return 0;

    int roomNumber = 0;
    const map<uint32_t, uint16_t>& guests = hotel_guests_.at(hotel_name);
