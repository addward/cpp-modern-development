...
vector<typename RandomIt::value_type> pool;
  ...
  vector<size_t> pool_idx(last - first);
  iota(pool_idx.begin(), pool_idx.end(), 0);
  size_t pos = 0;
  for (auto it = first; it != last; ++it) {
    //size_t pos = (it - first + step_size - 1) % pool.size();
    *it = move(pool[pool_idx[pos]]);
    pool_idx.erase(pos + pool_idx.begin());
    if (pool_idx.empty()) break;
    pos = (pos + step_size - 1) % pool_idx.size();
    //cout << "Pos: " << pos << ", Idx:" << pool[pos] << endl;
  }
}
...
void TestIntVector() {
  const vector<int> numbers = MakeTestVector();
  {
    ...
    vector<string> numbers_copy(100'000, "asdfasdfasdf");
    {LOG_DURATION("1000");
